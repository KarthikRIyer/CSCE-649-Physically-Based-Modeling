...
SpringyCube::~SpringyCube() = default;

void SpringyCube::tare() {
    for (const auto & particle : particles) {
        particle->tare();
    }
}

void SpringyCube::reset() {
    for (const auto & particle : particles) {
        particle->reset();
    }
    int posBufIndex = 0;
...
std::pair<std::vector<Eigen::Vector3d>, std::vector<Eigen::Vector3d >> SpringyCube::getVelAcc(double h, std::vector<std::shared_ptr<IForceField>>& forceFields,
...
void SpringyCube::step(double h, std::vector<std::shared_ptr<IForceField>>& forceFields, SimParams& simParams, std::vector<std::shared_ptr<Shape> >& shapes) {
//    for (const auto & particle : particles) {
//        particle->f = Eigen::Vector3d(0.0, 0.0, 0.0);
//        for (int i = 0; i < forceFields.size(); ++i) {
//            std::shared_ptr<IForceField> forceField = forceFields[i];
...
double SpringyCube::sgn(double x) {
    if (x > 0) return 1;
    if (x < 0) return -1;
    return 0;
}

void SpringyCube::detectEdgeCollision(std::shared_ptr<Edge> edge, std::vector<std::shared_ptr<Shape>> &shapes,
...
void SpringyCube::detectCollision(std::shared_ptr<Particle> particle, std::vector<std::shared_ptr<Shape> >& shapes, SimParams& simParams, double h) {
    if (particle->fixed) {
        return;
    }
...
void SpringyCube::cleanupBuffers() {
    glDeleteBuffers(1, &posBufID);
    glDeleteBuffers(1, &eleBufID);
    glDeleteVertexArrays(1, &VAO);
}

void SpringyCube::draw(const std::shared_ptr<Program> prog) const {
    GLSL::checkError(GET_FILE_LINE);
    glBindVertexArray(VAO);
